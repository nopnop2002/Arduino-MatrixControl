...
void setup() {
  ...
  while(1) {
    startMillis = millis();
    addr = showBitmap(bitmap[0], 8, startRow, numDevices, false, LEFTRIGHT);
    lc.pushAll();
    endMillis = millis();
    diffMillis = endMillis - startMillis;
    if (addr == numDevices) break;
    startRow++;
    delayMillis = interval - diffMillis;
    if (delayMillis > 0) delay(delayMillis);
  }
  lc.clearDisplay(numDevices-1);
  delay(5000);
#endif
  ...
  while(1) {
    startMillis = millis();
    addr = showBitmap(bitmap[0], 8, startRow, numDevices, true, LEFTRIGHT);
    lc.pushAll();
    //Serial.print("addr=");
    //Serial.println(addr);
    endMillis = millis();
...
#if 1
  startRow = -7;
  lastRow = numDevices * 8;
  while(1) {
    startMillis = millis();
    for(int index=0;index<numDevices;index++) {
      showBitmap(bitmap[index], 8, startRow-(8*index), numDevices, false, LEFTRIGHT);
    }
    addr = showBitmap(bitmap[numDevices], 8, startRow-(8*numDevices), numDevices, false, LEFTRIGHT);
    lc.pushAll();
    endMillis = millis();
    diffMillis = endMillis - startMillis;
    //Serial.print("addr=");
    ...
    if (addr == numDevices) break;
    startRow++;
    if (startRow == lastRow && stringPos < stringLen) {
      for(int index=0;index<numDevices;index++) {
  ...
  while(1) {
    startMillis = millis();
    for(int index=0;index<numDevices;index++) {
      showBitmap(bitmap[index], 8, startRow-(8*index), numDevices, true, LEFTRIGHT);
    }
    addr = showBitmap(bitmap[numDevices], 8, startRow-(8*numDevices), numDevices, true, LEFTRIGHT);
    lc.pushAll();
    endMillis = millis();
    diffMillis = endMillis - startMillis;
    //Serial.print("addr=");
